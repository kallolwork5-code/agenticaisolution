...
def get_upload_history():
    """Get upload history with dummy data"""
    
    # Generate dummy upload history
    history = [
        UploadHistoryItem(
            id="1",
            fileName="customer_data.csv",
            fileSize=2048576,
            uploadDate=datetime(2024, 12, 15, 10, 30, 0),
            classification="Customer Data",
            storageLocation="sqlite",
            recordCount=15420,
            status="success",
            aiSummary="Customer dataset with demographics and purchase history. High data quality detected with 98% completeness."
        ),
        UploadHistoryItem(
            id="2", 
            fileName="product_reviews.json",
            fileSize=1536000,
            uploadDate=datetime(2024, 12, 14, 14, 22, 0),
            classification="Text Data",
            storageLocation="chromadb",
            recordCount=8750,
            status="success",
            aiSummary="Product review text data suitable for sentiment analysis. Contains rich customer feedback with emotional indicators."
        ),
        UploadHistoryItem(
            id="3",
            fileName="transaction_log.csv", 
            fileSize=5242880,
            uploadDate=datetime(2024, 12, 13, 9, 15, 0),
            classification="Transaction Data",
            storageLocation="sqlite",
            recordCount=45230,
            status="success",
            aiSummary="Financial transaction records with payment methods and amounts. Suitable for fraud detection and spending analysis."
        ),
        UploadHistoryItem(
            id="4",
            fileName="rate_cards_2024.xlsx",
            fileSize=892416,
            uploadDate=datetime(2024, 12, 12, 16, 45, 0),
            classification="Rate Card Data", 
            storageLocation="sqlite",
            recordCount=2340,
            status="success",
            aiSummary="Payment processing rates and fees structure. Contains MDR rates, SLA requirements, and routing preferences."
        ),
        UploadHistoryItem(
            id="5",
            fileName="routing_rules.json",
            fileSize=156789,
            uploadDate=datetime(2024, 12, 11, 11, 30, 0),
            classification="Routing Data",
            storageLocation="chromadb", 
            recordCount=567,
            status="processing",
            aiSummary="Payment routing logic and decision rules. Currently being processed for optimization recommendations."
        )
    ]
