    ...
    try:
        ...
        if is_document:
            ...
            upload_record = await store_upload_record({
                ...
                "full_summary": document_result.get('summary', 'Document processed successfully')  # Store full summary
            })
...
async def store_upload_record(record_data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        # Generate AI summary from insights or use full summary for documents
        if record_data.get('full_summary'):
            ai_summary = record_data['full_summary'][:1000]  # Truncate for database storage
        else:
...
async def get_document_summary(upload_id: str):
    """Get full document summary for download"""
    ...
    try:
        ...
        if upload.classification != "document":
            raise HTTPException(status_code=400, detail="Not a document upload")
        
        # Get full summary from AI insights
        full_summary = upload.ai_summary
        ai_insights = []
        
        try:
            if upload.ai_insights:
                ai_insights = json.loads(upload.ai_insights)
                # Look for the full summary in insights
                for insight in ai_insights:
                    if insight.get('type') == 'summary' and insight.get('description'):
                        full_summary = insight['description']
                        break
        except Exception as e:
...
@router.get("/summary/{upload_id}/download")
async def download_document_summary(upload_id: str):
    """Download document summary as a properly formatted Word document"""
    ...
    try:
        ...
        summary_data = await get_document_summary(upload_id)
        ...
        # Add subtitle
        subtitle = doc.add_paragraph('Generated by CollectiSense AI')
        subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
        
        # Add document information
        doc.add_paragraph()
        doc.add_paragraph(f"File Name: {summary_data['file_name']}")
        doc.add_paragraph(f"Upload Date: {summary_data['upload_date'][:10]}")
        doc.add_paragraph(f"Classification: {summary_data['classification'].title()}")
        ...
        # Add metadata
        doc.add_paragraph()
        doc.add_heading('Processing Metadata', level=1)
        doc.add_paragraph(f"File Size: {summary_data['metadata']['file_size']:,} bytes")
        doc.add_paragraph(f"Document Chunks: {summary_data['metadata']['record_count']}")
        doc.add_paragraph(f"Storage Location: {summary_data['metadata']['storage_location'].upper()}")
        doc.add_paragraph(f"Processing Confidence: {summary_data['metadata']['confidence']:.1%}")
        ...
        # Create filename
        base_name = summary_data['file_name'].rsplit('.', 1)[0]
        ...
        content = f"""DOCUMENT SUMMARY REPORT
Generated by CollectiSense AI

File: {summary_data['file_name']}
Upload Date: {summary_data['upload_date']}
Classification: {summary_data['classification']}

{summary_data['summary']}

METADATA:
- File Size: {summary_data['metadata']['file_size']} bytes
- Chunks: {summary_data['metadata']['record_count']}
- Storage: {summary_data['metadata']['storage_location']}
- Confidence: {summary_data['metadata']['confidence']:.1%}
"""
